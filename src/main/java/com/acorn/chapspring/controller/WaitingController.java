...
public class WaitingController {
    ...
    public String waitingStatus(Model model,
        ...
        List<UsersWaitingDto> waitings = waitingService.getUserWaitings( storeNum,loginUser.getUserId());

        model.addAttribute("waitings", waitings);
        model.addAttribute("storeswaiting", storeswaiting);
        return "waiting/waitingStatus";
    }
    @GetMapping("/{storeNum}/ajaxWaitingStatus.do")
    public @ResponseBody Map ajaxWaitingStatus(
                                @SessionAttribute UserDto loginUser,
                                @PathVariable int storeNum
                                ){
        StoresWaitingDto storeswaiting = storesWaitingService.getStoreWaitingByStoreNum(storeNum);
        List<UsersWaitingDto> waitings = waitingService.getUserWaitings( storeNum,loginUser.getUserId());
        UsersWaitingDto loginUserWaiting = waitingService.getLoginUserWaiting( storeNum,loginUser.getUserId());

        Map<String, Object> reponseObj=new HashMap<>();
        reponseObj.put("storeswaiting",storeswaiting);
        reponseObj.put("waitings",waitings);
        reponseObj.put("loginUserWaiting",loginUserWaiting);
        return reponseObj;
    }
